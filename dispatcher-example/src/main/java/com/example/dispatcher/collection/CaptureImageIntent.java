...
import com.example.dispatcher.MainActivity;
...
public class CaptureImageIntent extends Intent {
    ...
    public CaptureImageIntent(@NonNull Context context) {
        ...
        if (tempFile == null) {
            Log.d(MainActivity.TAG, "Temp file is null, consult to https://github.com/hendraanggrian/dispatcher for correct configuration");
        } else {
    ...
    private static File createTempFile(@NonNull Context context) {
        try {
            ...
            if (parent.exists()) {
                Log.d(MainActivity.TAG + " [1/3]", "parent folder already exists, skipping...");
            } else {
                Log.d(MainActivity.TAG + " [1/3]", "parent folder not found, creating...");
                parent.mkdirs();
            }
            ...
            if (file.exists()) {
                file.delete();
                Log.d(MainActivity.TAG + " [2/3]", "temp file found, deleting...");
            } else {
                Log.d(MainActivity.TAG + " [2/3]", "temp file does not exist, skipping...");
            }

            final boolean isFileCreated = file.createNewFile();
            Log.d(MainActivity.TAG + " [3/3]", isFileCreated ? "temp file created!" : "unable to create file!");
            return file;
